...
def gen_tweet(inputtext):
    ...
    try:
        ...
        cleaned_txt = load_doc("tweets.txt_clean.txt")
        ...
        model = load_model_with_compatibility('troll_model.h5')
        
        if model is None:
            return "Error: Could not load the troll tweet model. The model may be incompatible with this version of TensorFlow."
        ...
        error_msg = f"Error in troll tweet generation: {str(e)}"
        print(error_msg)
        return error_msg
