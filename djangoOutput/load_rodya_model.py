...
def gen_crime(inputtext):
    ...
    try:
        ...
        cleaned_txt = load_doc("prestuplenie_i_nakazanie.txt_clean.txt")
        ...
        # Use a reasonable sequence length (cap at 50 to avoid memory issues)
        target_length = min(min_length, 50)
        ...
        model = load_model_with_compatibility('raskolnikov_model.h5')
        
        if model is None:
            return "Error: Could not load the Crime & Punishment model. The model may be incompatible with this version of TensorFlow."
        ...
        error_msg = f"Error in Crime & Punishment text generation: {str(e)}"
        print(error_msg)
        return error_msg
