...
def gen_yelp_review(inputtext):
    ...
    try:
        ...
        model = load_model_with_compatibility('yelp_model.h5')
        
        if model is None:
            return "Error: Could not load the Yelp review model. The model may be incompatible with this version of TensorFlow."
        ...
        error_msg = f"Error in Yelp review generation: {str(e)}"
        print(error_msg)
        return error_msg
