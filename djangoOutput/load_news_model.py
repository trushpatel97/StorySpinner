...
def gen_news(inputtext):
    ...
    try:
        ...
        cleaned_txt = load_doc("news.txt_clean.txt")
        ...
        model = load_model_with_compatibility('news_model.h5')
        
        if model is None:
            return "Error: Could not load the news model. The model may be incompatible with this version of TensorFlow."
        ...
        error_msg = f"Error in news generation: {str(e)}"
        print(error_msg)
        return error_msg
