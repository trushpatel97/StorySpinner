...
def gen_lovecraft(inputtext):
    ...
    try:
        ...
        cleaned_txt = load_doc("lovecraft.txt_clean.txt")
        ...
        model = load_model_with_compatibility('lovecraft_model.h5')
        
        if model is None:
            return "Error: Could not load the Lovecraft model. The model may be incompatible with this version of TensorFlow."
        ...
        error_msg = f"Error in Lovecraft text generation: {str(e)}"
        print(error_msg)
        return error_msg
